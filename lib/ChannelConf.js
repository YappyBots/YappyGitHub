...
class ChannelConf extends Collection {
  ...
  */
  init(bot) {
    ...
    db.open(dbPath).then(() => {
      ...
      for (const row of rows) this.set(row.channel_id, row);
    });
  ...
  */
  add(channel_id, repo) {
    return new Promise((resolve, reject) => {
      if (this.has(channel_id)) return reject('Channel already has github repo webhooks configured');
      db.open(dbPath).then(() => {
        db.run(`INSERT INTO "channel_configs" (channel_id, repo) VALUES (?, ?)`, [channel_id, repo]).then(() => {
          this.set(channel_id, { channel_id, repo })
  ...
  */
  delete(channel_id) {
    return new Promise((resolve, reject) => {
      if (!this.has(channel_id)) return reject('Channel doesn\'t have github repo events to delete');
      db.open(dbPath).then(() => {
        db.run(`DELETE FROM channel_configs WHERE channel_id = ${channel_id}`).then(() => {
          this.delete(channel_id);
          resolve(this);
        }).catch(reject);
      })
    });
