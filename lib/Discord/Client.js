...
class Client extends DiscordClient {
  ...
  async runCommandMessage(msg) {
    ...
    const cmd =
      this.commands.get(command) || this.commands.get(bot.aliases.get(command));
  ...
  async registerCommands() {
    ...
    const guildCommands = commands.filter((cmd) => cmd.conf.permLevel !== 2);
    const ownerCommands = commands.filter((cmd) => cmd.conf.permLevel === 2);

    // TODO change to global for prod
    ...
    // TODO separate with owner-only commands in specific guild
  }
}
