...
class Client extends DiscordClient {
  ...
  */
  addCommand(Cmd, file) {
    try {
      ...
      this.emit('error', `Command | ${f || Cmd.name}`, error);
    }
  }
  ...
  */
  runCommand(msg) {
    ...
    if (!msg.mentions.has(this.user) && !msg.content.startsWith(prefix) && !msg.content.startsWith(this.prefix)) return;

    const content = this.getMsgContent(msg, prefix);
    const command = content.split(' ')[0].toLowerCase();
    const args = content.split(' ').slice(1);

    const middleware = this.middleware.array().sort((a, b) => b.priority - a.priority);
    let i = 0;
  ...
  */
  getMsgContent(message, prefix) {
    ...
    return (content.startsWith(prefix) && content.replace(prefix, '')) ||
    (mentions.users.size && mentions.users.first().equals(this.user) && content.split(' ').slice(1).join(' ')) ||
