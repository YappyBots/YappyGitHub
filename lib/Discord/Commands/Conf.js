...
class ConfCommand extends Command {
  ...
  async _view(interaction, type, conf) {
    const { guild, channel } = interaction;
    let embed = new this.embed().setColor('#FB9738').setAuthor({
      name: guild.name,
    ...
    if (type === 'guild') {
      ...
      embed
        ...
        .addFields([
          ...
          {
            name: 'Webhook Secret (secret)',
            value: `||\`${await conf.getSecret()}\`||`,
            inline: true,
          },
          {
            name: 'Use Embed (useEmbed)',
            value: this.format(conf.get('useEmbed')),
            inline: true,
          },
          {
            name: 'Ignore Unknown Events (ignoreUnknown)',
            value: this.format(conf.get('ignoreUnknown')),
            inline: true,
          },
          {
            name: `Events [${conf.get('eventsType')}]`,
            value: this.format(conf.get('eventsList')),
            inline: true,
          },
          {
            name: `Users [${conf.get('usersType')}]`,
            value: this.format(conf.get('usersList')),
            inline: true,
          },
          {
            name: `Branches [${conf.get('branchesType')}]`,
            value: this.format(conf.get('branchesList')),
            inline: true,
          },
          {
            name: `Repositories [${conf.get('reposType')}]`,
            value: this.format(conf.get('reposList')),
            inline: true,
          },
        ]);
    }
          ...
          .addFields([
            {
              ...
              value: this.format(
                value,
                conf.casts && conf.casts[key] === 'boolean'
              ),
            },
          ]);
  ...
  async _filter(interaction, conf) {
    ...
    if (!action || action === Actions.INVALID) {
      ...
      const embed = new this.embed()
        .setTitle(`#${interaction.channel.name}'s ${type}ed ${obj}`)
        .addFields(
          list.map((e) => ({ name: e, value: '\u200B', inline: true }))
        );
