...
class ConfCommand extends Command {
  ...
  _view(interaction, type, conf) {
    ...
    if (type === 'guild') {
      embed = embed
        .addField('Repo', this.format(conf.get('repo')), true);
    } else {
      embed = embed
        .setDescription(`This is your current channel's configuration.`)
        .addField('Initialized Repos', this.format(conf.getRepos()), true)
        .addField('Initialized Org', this.format(conf.getOrg()), true)
  ...
  _set(interaction, key, value, type, conf) {
    ...
    if (key !== 'prefix' && arrRegex.test(value)) value = value.split(arrRegex);
