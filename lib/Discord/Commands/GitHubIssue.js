...
class GitHubIssue extends Command {
  ...
  async run(msg, args) {
    const repo =
      (await Channel.findOrCreate(msg.channel)).get('repo') ||
      (await Guild.findOrCreate(msg.guild)).get('repo');
  ...
  async issue(msg, repository, args) {
    ...
    try {
      ...
      const embed = new this.embed()
        ...
        .setAuthor(issue.user.login, issue.user.avatar_url, issue.user.html_url)
