...
class GithubInitCommand extends Command {
  ...
  async run(msg, args) {
    ...
    if (isPrivate) {
      ...
      return ChannelConfig.addRepoToChannel(channelid, repoFullName).then(() => {
        let embed = this._successMessage(repository.repo);
        return workingMsg.edit({ embed });
      });
    }

    return Github.getRepo(repoFullName).then(data => {
  ...
  _successMessage(repo) {
    return {
      ...
      description: [
        'The repository must a webhook pointing to <https://www.yappybots.ga/github>',
