...
class GithubRemoveCommand extends Command {
  ...
  async run(msg, args) {
    ...
    if (conf.repos.length > 1 && repo.repo)
      repoFound = conf.repos.filter(
    ...
    if (args[0] && !repoFound) {
      return this.commandError(
        msg,
        `This channel doesn't have github events for **${repo.repo ||
          args[0]}**!`
      );
    } else if (conf.repos.length && conf.repos.length > 1 && !repoFound) {
      return this.commandError(
        msg,
        `Specify what github repo event to remove! Current repos: ${conf.repos
          .map(e => `**${e}**`)
          .join(', ')}`
      );
    }
