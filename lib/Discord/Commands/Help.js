...
class HelpCommand extends Command {
  ...
  run(msg, args) {
    ...
    if (!commandName) {
      ...
      let commandsForEveryone = commands.filter(
        (e) => !e.conf.permLevel || e.conf.permLevel === 0
      );
      ...
      let embed = new this.embed()
        ...
        .setDescription(
          `Use \`@Yappy help <command>\` for details\n\u200B`
        )
      ...
      let command =
        ...
        (this.bot.aliases.has(commandName)
      ...
      let embed = new this.embed()
        ...
        .setFooter(this.bot.user.username, this.bot.user.avatarURL())
        .addField('Usage', `\`@Yappy ${command.help.usage}\``);

      if (command.conf.aliases && command.conf.aliases.length)
        embed.addField(
          'Aliases',
          command.conf.aliases.map((e) => `\`${e}\``).join(', '),
          true
        );
