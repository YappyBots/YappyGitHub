...
class UpdateCommand extends Command {
    ...
    .then(info => {
      ...
      return this.installDeps(info).then(stdouts =>
        this.addFieldToEmbed(message, embedData, {
          name: 'NPM',
          value: stdouts.length > 0 ? stdouts.map(stdout => `\`\`\`sh\n${stdout.slice(0, 1000)}\n\`\`\``).join('\n') : 'No packages were updated',
        })
      );
    })
  ...
  async installDeps(data) {
    let stdouts = [
      data.install.length && await this.exec(`npm i --no-progress ${data.install.map(e => `${e[0]}@${e[1]}`).join(' ')}`),
      data.update.length && await this.exec(`npm upgrade --no-progress ${data.update.map(e => `${e[0]}@${e[1]}`).join(' ')}`),
      data.remove.length && await this.exec(`npm rm --no-progress ${data.remove.map(e => e[0]).join(' ')}`),
    ];
