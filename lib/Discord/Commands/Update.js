...
class UpdateCommand extends Command {
  ...
  async run(msg, args) {
    ...
    const reply = await msg.channel.send({ embeds: [embedData] });

    return this.exec('git pull')
      .then((stdout) => {
        if (stdout.includes('Already up-to-date')) {
          return this.addFieldToEmbed(reply, embedData, {
            name: 'Git Pull',
      ...
      .then((info) => {
        ...
        return this.installDeps(info).then((stdouts) =>
          this.addFieldToEmbed(reply, embedData, {
            name: 'NPM',
            value:
              stdouts.length > 0
                ? stdouts
      ...
      .catch((err) => {
        ...
        return this.commandError(reply, err);
      });
  }
  ...
  addFieldToEmbed(reply, data, field) {
    data.fields.push(field);
    return reply.edit({ embeds: [data] });
  }
