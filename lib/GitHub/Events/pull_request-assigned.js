...
class PullRequestAssigned extends EventResponse {
  constructor(...args) {
    super(...args, {
      description:
        'This response is fired whenever a person is assigned to a pull request.',
    });
  }
  embed(data) {
    ...
    return {
      color: '#149617',
      title: `Assigned ${assignee.login} to #${pr.number} (**${this.escape(pr.title)}**)`,
      url: pr.html_url,
    };
  }
  text(data) {
    ...
    return [
      ...
      } to **#${pr.number}** (${this.escape(pr.title)})`,
      `<${pr.html_url}>`,
    ];
  }
}
