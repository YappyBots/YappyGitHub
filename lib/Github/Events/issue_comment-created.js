...
class IssueComment extends EventResponse {
  constructor(...args) {
    super(...args, {
  ...
  embed(data) {
    ...
    return {
      ...
      title: `Created a comment on ${isIssue ? 'issue' : 'pull request'} #${data.issue.number}: **${data.issue.title}**`,
  ...
  text(data) {
    ...
    return [
      `💬  **${actor.login}** commented on ${isComment ? 'issue' : 'pull request'} **#${issue.number}** (${issue.title})`,
      `<${comment.html_url}>`,
    ].join('\n');
  }
}
