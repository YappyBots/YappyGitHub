...
class IssueCommentEdited extends EventResponse {
  ...
  embed(data) {
    ...
    return {
      ...
      title: `Edited a comment on ${isIssue ? 'issue' : 'pull request'} #${data.issue.number}: **${data.issue.title}**`,
  ...
  text(data) {
    ...
    return [
      `💬  **${actor.login}** edited a comment on ${isComment ? 'issue' : 'pull request'} **#${issue.number}** (${issue.title})`,
      `<${comment.html_url}>`,
    ].join('\n');
  }
}
