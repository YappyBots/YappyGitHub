...
class Github {
  ...
  */
  getOrgRepos(org) {
    ...
    return this.gh.repos.listForOrg({
      org,
    }).then(res => res.data);
  }
  ...
  */
  search(type, data) {
    ...
    return this.gh.search[type](data instanceof String ? {
      q: data,
  ...
  */
  getGithubError(err) {
    return JSON.parse(err.message && err.message.startsWith('{') ? err.message : err);
  }
   ...
   */
  isGithubError(err) {
    return err && err.headers && err.headers.server && err.headers.server === this.Constants.HOST;
  }
