...
class Logger {
  ...
  constructor() {
    ...
    this._token = process.env.DISCORD_TESTING_BOT_TOKEN || process.env.TOKEN;
  }
  ...
  error(msg) {
    if (typeof msg == 'object' && msg.stack) {
      msg.stack = msg.stack.replace(__dirname, './').replace(new RegExp(this._token, 'g'), 'TOKEN_WAS_HERE');
    }
    this.log('error', msg);
  }
