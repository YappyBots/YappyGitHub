...
class ErrorLogger {
  ...
  init(bot) {
    bot.fetchWebhook(process.env.DISCORD_WEBHOOK_ID, process.env.DISCORD_WEBHOOK_TOKEN).then(webhook => this.webhook = webhook);

    process.on('uncaughtException', e => this.error(e));
    process.on('unhandledRejection', e => this.error(e));

    bot.on('error', e => this.error(e));
