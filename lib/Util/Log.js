...
class Log {
  ...
  error(error, ...args) {
    ...
    if (error instanceof Error) this.raven.captureException(error); else this.raven.captureMessage(typeof error === 'object' ? util.inspect(error) : error);
    this.logger.error(error, ...args);
    return this;
  }
  warn(warn, ...args) {
    this.logger.warn(warn, ...args);
    if (typeof warn === 'object') this.raven.captureException(warn, { level: 'warning' }); else this.raven.captureMessage(warn, { level: 'warning' });
    return this;
  }
  info(...args) {
