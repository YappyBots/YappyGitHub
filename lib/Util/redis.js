...
const client = redis.createClient(process.env.REDIS_HOST, process.env.REDIS_HOST);

exports.get = (db, key) => client.get(`${db}:${key}`);
exports.set = (db, key, value, expiry = -1) => client.set(`${db}:${key}`, value, {
    EX: expiry
});

exports.expire = (db, key, expiry) => client.expire(`${db}:${key}`, expiry);

exports.setHash = (db, key, data, expiry) => client.hSet(`${db}:${key}`, data)
...
.catch((err) => Log.error('Redis | Failed to connect', err));