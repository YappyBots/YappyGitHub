...
app.post(['/', '/github'], async (req, res) => {
  ...
  await Promise.all(
    channels.filter(Boolean).map(async (conf) => {
      ...
      const ignoredRepos = conf.get('ignoredRepos') || [];
      ...
      if (
        ignoredUsers &&
        (ignoredUsers.includes(actor.name) ||
          ignoredUsers.includes(actor.login))
      )
        return;
      if (ignoredBranches && branch && ignoredBranches.includes(branch)) return;
      if (ignoredRepos && ignoredRepos.includes(repo)) return;
