...
app.get('/setup', (req, res, next) => {
  ...
  res.send(
    `Error: ${error} // Description: ${error_description} // URI: ${decodeURIComponent(
      error_uri
    )}`
  );
});
...
app.use(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!channel?.channel_id) return next(404);
...
app.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    // console.log(`GET /app/installations/${githubId}`)
...
app.use('/setup/:id', (err, req, res, next) => {
  if (req.setupData) delete req.setupData.github_token;

  //  TODO error handling for async methods
