...
app.post(['/', '/github'], (req, res) => {
  ...
  if (!event || !data || !req.headers['x-github-delivery']) return res.status(403).send('Invalid data. Please use Github webhooks.');
...
app.use(addons.express.middleware(bot, require('./Models'), {
  ...
  host: process.env.BDPW_KEY ? 'https://www.yappybots.ga/beta/github' : `http://localhost:${port}`,
}));
