...
app.get('/setup/:id', async (req, res, next) => {
  ...
  redis.setHash('setup', id, [['state', state]]);
  ...
  res.redirect(
    `https://github.com/login/oauth/authorize?${querystring.stringify(query)}`
  );
});

app.get('/setup/:id/callback', async (req, res, next) => {
  const id = req.params.id;
  const { channel_id, state } = await redis.getHash('setup', id);
  ...
  res.redirect(`/setup/${id}/connect`);
});

app.get('/setup/:id/connect', async (req, res, next) => {
  const id = req.params.id;
  const { channel_id, github_token, github_login } = (setupData =
    await redis.getHash('setup', id));
  const ttl = await redis.ttl('setup', id);

  if (!channel_id) return next(404);
  if (!github_token) return res.redirect(`/setup/${id}`);

  const connections = await ChannelConnection.where(
    'channel_id',
    channel_id
  ).fetchAll();
  const githubApp = await GitHub.userOAuthRepositories(
    github_token,
    id,
    req.query.force !== undefined
  );

  res.render('setup.ejs', { connections, githubApp, setupData });
});

app.get('/setup/:id/connect/:type/:githubId', async (req, res, next) => {
  const { id, type, githubId } = req.params;
  const { channel_id, github_token } = (setupData = await redis.getHash(
    'setup',
    id
  ));
  ...
  if (connection) res.redirect(`/setup/${id}/connect`);

  const octokit = GitHub.fromOAuthToken(github_token);

  // TODO retrieve name from GitHub
