...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!repo && !installId)
      throw new BadRequestError('Could not find repository or installation data in request.');
    ...
    if (eventResponse === 'err') {
      return res.send('An error occurred while processing the event.');
...
const sendToChannels = async ({ channels, eventResponse, data }) => {
  ...
  const handleError = async (err, channel) => {
    ...
    const noAccess = [RESTJSONErrorCodes.MissingAccess, RESTJSONErrorCodes.MissingPermissions];
    ...
    if (noAccess.includes(err.code)) {
      ...
      return owner
        ...
        .catch((err) => {
          if ([...noAccess, RESTJSONErrorCodes.CannotSendMessagesToThisUser].includes(err.code)) return;
...
app.get(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    const channels = (await Promise.allSettled(ids.map(cache.channels.resolve)))
      .map((v) => v.value)
      .filter((c) => !c?.isDMBased() && c?.isTextBased());
