...
const handleChannelEndpoints = async (
  req,
  res,
  { channelConfigs, checkSecret = true }
) => {
  // Retrieve hook info
  const event = req.headers['x-github-event'];
  const delivery = req.headers['x-github-delivery'];
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const channels = (
      await Promise.all([
        repo &&
          LegacyChannelRepo.query((q) => q.where('name', repo)).fetchAll({
            withRelated: ['channel'],
          }),
        org &&
          LegacyChannelOrg.query((q) => q.where('name', org)).fetchAll({
            withRelated: ['channel'],
          }),
      ])
    )
    ...
    if (!channels.length) {
      return res.send(`${repo || org} : no legacy channels found`);
    }
