  ...
  if (Log.sentry) {
    return Sentry.withIsolationScope((scope) => {
      ...
      res.on('finish', () => {
        scope.addBreadcrumb({
          category: 'http.server',
          message: `${requestLabel} -> ${res.statusCode}`,
          data: {
            method: req.method,
            url: req.originalUrl || req.url,
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const connections = await ChannelConnection.query((q) => {
      // Seems like some events don't include installation data, which causes issues with the DB query.
      // However, they still include repository data, so we can match on that instead (won't match everything but better than nothing).
      if (installId) {
        q.where(function () {
      ...
      if (repoId) {
        const addRepoClause = installId ? 'orWhere' : 'where';

        q[addRepoClause](function () {
    ...
    if (eventResponse?.type === 'error') {
      return res
        .status(500)
        .send(
          `An error occurred while processing the event. Sentry ID: ${
            eventResponse.sentry ?? 'N/A'
          }`
        );
...
const annotateWebhookSentry = ({ delivery, event, endpoint }) => {
  if (!Log.sentry) return;

  Sentry.setTag('webhook.delivery', String(delivery));
  Sentry.setTag('webhook.event', String(event));
  Sentry.setTag('webhook.endpoint', endpoint);
};
...
const handleChannelEndpoints = async (
  ...
  res.send(
    escapeInput`${repo || org} : Received ${event}${actionText}, emitting to ${
      botChannels.length
    } channels (skipped ${skip.length} due to config mismatch)...`
  );
