...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const installAccId = get(data, 'installation.account.id');
    const installAccLogin = get(data, 'installation.account.login');
    const org = get(data, 'organization.login');

    if (!repo && !installAccId) {
      return res
        .status(403)
    ...
    const connections = await ChannelConnection.query((q) => {
      q.where(function () {
        ...
        this.where('github_id', installAccId);
      });
    ...
    Log.verbose(`GitHub | ${repo || installAccLogin} - ${event}${actionText}`);
