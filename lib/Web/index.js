...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { repo, org, repoId, installId, eventOwner, actionText } =
      getHookInfo(data);
...
app.post(
  ...
  asyncHandler(async (req, res, next) => {
    ...
    if (!event || !req.headers['x-github-delivery'] || (!repo && !org))
      return next(
        new BadRequestError('Invalid data. Please use GitHub webhooks.')
      );
...
app.use((err, req, res, next) => {
  ...
  res.format({
    ...
    text: () => res.type('txt').send(`${status} ${err.name} ${err.message}`),
  });
});
