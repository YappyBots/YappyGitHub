...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const installationLogin = get(data, 'installation.account.login');
    ...
    Log.verbose(`GitHub | ${repo || installationLogin} - ${event}${actionText}`);
    ...
    if (!eventResponse) {
      res.send(
        `${repo || installationLogin} : Ignoring ${event}${actionText}, this type of event is automagically ignored`
      );
      return;
    }

    res.send(
      `${repo || installationLogin} : Received ${event}${actionText}, emitting to ${channels.length} channels...`
    );
