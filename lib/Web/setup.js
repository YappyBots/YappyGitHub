...
module.exports = (app) => {
  ...
  app.use(
    ...
    asyncHandler(async (req, res, next) => {
      ...
      if (!channel?.channel_id) throw new NotFoundError('Unknown setup ID');
  ...
  app.use('/setup', (req, res, next) => {
    ...
    if (req.path.slice(-1) === '/' && req.path.length > 1) {
      ...
      res.redirect(301, `${process.env.WEB_HOST}/setup${safepath}${query}`);
    } else {
      next();
    }
  });
  ...
  app.get(
    ...
    asyncHandler(async (req, res, next) => {
      ...
      if (data.state !== state) {
        return next(new ForbiddenError('Invalid OAuth state'));
      }
      ...
      res.redirect(`${process.env.WEB_HOST}/setup/${id}/dashboard`);
    })
  );
  ...
  app.post(
    ...
    asyncHandler(async (req, res, next) => {
      ...
      if (!github_token)
        return res.redirect(`${process.env.WEB_HOST}/setup/${id}`);
  ...
  app.get(
    ...
    asyncHandler(async (req, res, next) => {
      ...
      if (!github_token)
        return res.redirect(`${process.env.WEB_HOST}/setup/${id}`);
      if (channel_id == -1)
        return res.redirect(`${process.env.WEB_HOST}/setup/${id}/purge`);
  ...
  app.post(
    ...
    asyncHandler(async (req, res, next) => {
      ...
      const githubId = Array.isArray(req.params.githubId)
        ? req.params.githubId.join('/')
        : req.params.githubId;
      ...
      if (!githubData)
        return res.redirect(
          `${process.env.WEB_HOST}/setup/${id}/refresh-github`
        );
  ...
  app.post(
    ...
    asyncHandler(async (req, res, next) => {
      ...
      if (type.startsWith('legacy')) {
        ...
        return res.redirect(`${process.env.WEB_HOST}/setup/${id}/dashboard`);
      }
  ...
  app.get('/setup/:id/:action/:type/:githubId', (req, res, next) => {
    const { action, type, githubId } = req.params;

    // show error for wrong HTTP method

    throw new MethodNotAllowedError(
      `Cannot ${action.toUpperCase()} ${type.replace('-', ' ')} "${githubId}" by directly accessing this URL -- please use the buttons provided.`
    );
  });
  ...
  app.post(
    ...
    asyncHandler(async (req, res, next) => {
      ...
      res.redirect(`${process.env.WEB_HOST}/setup/${id}/purge`);
    })
  );
